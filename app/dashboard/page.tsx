...
import { Newspaper, ShoppingCart, Eye, TrendingUp, IndianRupee, Clock, Loader2 } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function fetchDashboardData() {
      ...
      if (newsData) {
        setRecentActivity(newsData.slice(0, 5)); // top 5 recent

        const published = newsData.filter(n => n.status === 'published').length;
        const pending = newsData.filter(n => n.status === 'pending').length;
        const totalViews = newsData.reduce((sum, n) => sum + (n.views_count || 0), 0);

        // Fetch earnings (simulated for now, or you could sum transactions)
        const earned = newsData.filter(n => n.status === 'sold').reduce((sum, n) => sum + Number(n.reporter_price || 0), 0);
